...
def generate_recommendations(user_playlist: dict, user_id: int):
    ...
    """
    spotify_df = pd.read_csv('./recommendations/data/tracks.csv', skiprows=range(1, 170000), nrows=50000)
...
#    Get the year from the release_date column
    prepared_data['year'] = prepared_data['release_date'].apply(lambda x: x.split('-')[0])

    # Get the column names of the float variables
    float_cols = prepared_data.dtypes[prepared_data.dtypes == 'float64'].index.values
    
    ohe_cols = 'popularity'

    # create 5 point buckets for popularity 
    prepared_data['popularity_red'] = prepared_data['popularity'].apply(lambda x: int(x/5))

    # tfidf can't handle nulls so fill any null values with an empty list
    prepared_data['consolidates_genre_lists'] = prepared_data['consolidates_genre_lists'].apply(lambda d: d if isinstance(d, list) else [])
    ...
    filtered_playlist_df = create_necessary_outputs(user_playlist, prepared_data)
    ...
    feature_set_playlist_vector, feature_set_nonplaylist = generate_playlist_feature(feature_set, filtered_playlist_df, 1.09)    

    # Generate the top 40 song recommendations
    top_40_recommendations = generate_playlist_recos(prepared_data, feature_set_playlist_vector, feature_set_nonplaylist, user_id);
