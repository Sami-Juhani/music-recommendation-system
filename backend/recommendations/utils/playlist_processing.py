...
def create_necessary_outputs(spotify_playlist: dict, df: pd.DataFrame) -> pd.DataFrame:
    """ 
...
def generate_playlist_feature(complete_feature_set: pd.DataFrame, playlist_df: pd.DataFrame, weight_factor: float) -> Tuple[pd.Series, pd.DataFrame]:
    """ 
...
def generate_playlist_recos(df: pd.DataFrame, features: pd.Series, nonplaylist_features: pd.DataFrame, user_id: int) -> Union[Dict[str, str], pd.DataFrame]:
    """ 
    Pull songs from a specific playlist.
    ...
    non_playlist_df_top_40['url'] = non_playlist_df_top_40['id'].apply(lambda x: execute_spotify_api_request(user_id, f'{endpoint}/{x}')['album']['images'][1]['url'])
